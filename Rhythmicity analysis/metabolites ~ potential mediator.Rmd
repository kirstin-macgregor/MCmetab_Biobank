...
for (i in variables_loop) {
  ...
  data_loop_covar <- rename(data_loop, covar= j)  
  
  mod1 <- gam(var~  ethnicity_cat + s(MC.status, bs="cc") + s(age) + s(deprivationindex) + s(covar), data=data_loop_covar)
...
mod_out <- data.frame(matrix(ncol =22, nrow = 0))
...
#getwd()
#write.csv(mod_out, 'inflamCovarAdj_MC_metab_MCexcluded.csv')